...
transaction(receiver: Address) {
    ...
    prepare(signer: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, UnpublishCapability) &Account) {
        ...
        let recipient = getAccount(receiver).capabilities.get<&{FungibleToken.Receiver}>(/public/flowTokenReceiver)
        ...
        signer.storage.save(<-vault, to: /storage/flowTokenForwarder)

        // Link the new ForwarderPublic capability
        let tokenForwarderCap = signer.capabilities.storage.issue<&{FungibleToken.Receiver}>(
            /storage/flowTokenForwarder
        )
        signer.capabilities.publish(tokenForwarderCap, at: /public/flowTokenForwarder)
    }
}
